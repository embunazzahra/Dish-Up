...
async function searchRecipeByBookmark(user) {
  ...
  if (result.message == "success") {
    ...
    const query = `SELECT recipes.recipe_name, recipes.recipe_id, count(saved_recipes.recipe_id),
    recipes.user_id,users.username
    FROM recipes
    INNER JOIN users
    ON recipes.user_id = users.user_id
    left join saved_recipes ON recipes.recipe_id = saved_recipes.recipe_id WHERE to_tsvector(recipe_name) @@ to_tsquery('${searchTitleWith}') AND  recipes.recipe_id NOT IN (SELECT recipe_id FROM saved_recipes WHERE user_id = ${user.user_id}) GROUP BY recipes.recipe_name, recipes.recipe_id, recipes.user_id,users.username ORDER BY count(saved_recipes.recipe_id) DESC`;
    let finalResult = await db.query(query);
    if (finalResult.rowCount > 0) {
      return { message: "success", data: finalResult.rows };
    }
  }
  const query = `SELECT recipes.recipe_id,recipes.recipe_name,recipes.user_id,users.username, count(saved_recipes.recipe_id)
      FROM recipes
      INNER JOIN users
      ON recipes.user_id = users.user_id
      INNER JOIN saved_recipes
      ON recipes.recipe_id = saved_recipes.recipe_id
      GROUP BY recipes.recipe_id,recipes.recipe_name,recipes.user_id,users.username ORDER BY count(saved_recipes.recipe_id) DESC LIMIT 6;`;
  let allresult = await db.query(query);
  if (result.rowCount == 0) {
    return { message: "No recipe exist." };
  } else {
    return { message: "success", data: allresult.rows };
  }
}
